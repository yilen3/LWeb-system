<template>
  <el-dialog v-bind="$attrs" v-model="tempShow" :style="{
    ...
    <template #[slotName]="slotProps" v-for="(slot, slotName) in $slots" :key="slotName">
      <slot :name="slotName" v-bind="{ set: mSet, ...slotProps }" />
    </template>
    <template #header>
      <slot name="header">
        <div class="xbc w-full">
          <slot name="title">
            <span>{{ mSet && mSet.title }} <slot name="subTitle">{{ title }}</slot></span>
          </slot>
          <!-- 调试的set属性代码 -->
          <div class="xrc gap-3">
              ...
              <slot name="code">
                <el-tabs modelValue="JSON数据" type="card" tab-position="top">
                  ...
                  <el-tab-pane label="el-form-span" name="el-form-span" lazy>
                    <el-radio-group v-model="mSet.span" class="w-340px gap-1 flex-wrap">
            ...
            <!-- <el-icon v-if="!hiddenFullscreen && !mSet.fullscreen" :size="1.5" class="cursor-pointer"
...
const show: any = defineModel('modelValue')
const dialogConfig = defineModel('set', { type: Object, default: {} })
console.log('%c [ dialogConfig ]-69', 'font-size:13px; background:pink; color:#bf2c9f;', dialogConfig)
...
const tempSet = ref<any>({})
dialogConfig.value = useConfirmDialog(show)
